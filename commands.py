...
TX_2400_PACKETS_EXECUTABLE = "/home/pi/code/radio/build/Lora_tx_packets"
S_BAND_FREQ = 2400
...
def send_packets_2400(filepath: str, packets: list[int]):
    '''
    ...
    if not os.path.isfile(filepath): raise FileNotFoundError(f"{filepath} not found!")
    subprocess.Popen(
        f"{TX_2400_PACKETS_EXECUTABLE} {S_BAND_FREQ} {filepath} {','.join([str(packet) for packet in packets])}", 
        shell=True
    )
